  ...
  <section class="main-slider w-100 cl-white">
    ...
    <div class="hidden-tb hidden-xs desktopslider">
      ...
      <no-ssr>
        ...
        <carousel :per-page="1" :autoplay="true" :loop="true" :autoplayTimeout="8000" :navigationEnabled="true"
    ...
    <div class="hidden-sm hidden-md hidden-lg hidden-xl mobileslider">
      ...
      <no-ssr>
        <carousel :per-page="1" :autoplay="true" :loop="true" :autoplayTimeout="8000" pagination-active-color="#c0c9d5"
                  pagination-color="#eef1f5">
          <slide v-for="(slide, index) in bannerData" :key="index" v-if="slide.status != 0">

            <!-- 2  button layout -->
            <div v-if="slide.title" class="container w-100"
                 v-lazy:background-image="getThumbnail(slide.image_mobile,0,0,'mageplazabanner')">
              <router-link :to="localizedRoute(slide.url_banner)">
                <div class="row center-xs">
...
export default {
  ...
  methods: {
    async fetchData () {
      let jsonUrl = this.baseUrl + '/pub/media/banners.json';
      this.getJSONP('https://haider.pw/json.php?url=' + jsonUrl + '&callback=?', (data) => {
