        ...
        <div class="row">
          <div class="col-md-12 pl20 pr20">
            ...
            <p
              v-html="
                this.$t(
                  'You will receive an order confirmation email with details of your order and once dispatched a link to track your parcel’s progress.'
                )
              "
            />
            <p
              v-html="
                this.$t(
                  'E-mail us at <b>contactus@w10.world</b> with any questions or suggestions on how we could improve products or shopping experience'
                )
              "
            />
            <h3>
            ...
            <p
              v-html="
                this.$t(
                  'You can log into your account using the e-mail and password setup earlier. On your account you can <b>edit your profile data</b> and check <b>history of transactions</b>.</b>'
                )
              "
            />
          </div>
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      const response = await fetch(`${orderDetail_URL}`, {
        method: "post",
        mode: "cors",
        headers: {
      ...
      console.log(
        "response2",
        this.subTotal,
        jsonRes.result.message.order.subtotal
      );
    } catch (error) {
      console.log(error);
    }
    ...
    this.$gtm.trackEvent({
      ...
      value: this.subTotal,
      currency: "GBP",
      transaction_id: this.$route.params.orderId
  ...
  computed: {
    ...
    ...mapState({
      lastOrderConfirmation: (state) =>
        get(state, "order.last_order_confirmation.confirmation") || {},
      checkoutPersonalEmailAddress: (state) =>
        state.checkout.personalDetails.emailAddress,
    }),
    platformTotal() {
        ...
        ecommerce: {
          ...
          purchase: {
            actionField: {
              ...
              affiliation: "W10",
              revenue:
                cart && cart.base_grand_total ? cart.base_grand_total : "",
              tax: cart && cart.base_tax_amount ? cart.base_tax_amount : "",
              shipping:
                cart && cart.base_shipping_amount
                  ? cart.base_shipping_amount
                  : "",
              coupon: "",
            },
            products: this.$store.state.cart.cartItems.map((product) =>
              getProduct(product)
            ),
          },
        },
